...
public class PersonaController {
	...
	public Response insert(Persona persona) {
		...
		if ( violaciones.isEmpty() ) {
			...
			for (ConstraintViolation<Persona> violation : violaciones) {
				responseBody.add( violation.getPropertyPath() + ": " + violation.getMessage() );
			}
			response = Response.status(Status.BAD_REQUEST).entity(responseBody).build();
		}
